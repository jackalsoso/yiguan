  ...
  <div class="content" :class="{ 'send-up':is_up_img, 'send-face':is_face }">
    <div class="back-fixed">
      <div class="back-top">
        ...
        <img
          class="more"
          @click="$router.push({ path: `/setgroupWhat`,query:{type:0} })"
      ...
      <div class="chat_list" v-for="(val,lIndex) in messageList" :key="lIndex">
        ...
        <div class="other" v-else>
          <div class="img_box">
            <img
              class="icon"
              :src="val.headimg? val.headimg : '@/static/img/moren.png'"
              alt
              @click="$router.push({ path: '/goodsInfo' ,query:{type:2,member_id: val.sent_id,f_pk_id:$route.query.f_pk_id}})"
            />
          </div>
          <span v-if="val.content_type == 0">
            <label class="back-text" v-for="(val, i) in face_analysis(val.content)" :key="i">
    ...
    <div class="send-msg" :class="{ 'send-up':is_up_img, 'send-face':is_face }">
      <div class="input">
        ...
        />
        <img
          src="@/static/img/jianpan.png"
          alt
          class="jianpan"
          v-show="is_speak"
          @click="is_speak = false"
        />
        ...
        <img
          ...
          alt
          class="face"
          @click="is_face = !is_face,is_up_img = false"
        />
        <img
          src="@/static/img/jia.png"
          alt
          class="up"
          @click="is_up_img = !is_up_img,is_face = false"
        />
      </div>
      <div class="upload" v-if="is_up_img">
        <div class="media">
          ...
          />
          <div>
            <img src="@/static/img/up-img.png" alt />
          </div>
          <span>图片</span>
        </div>
        <div class="media">
          <div @click="shootVideo">
            <img src="@/static/img/paishe.png" alt />
          </div>
          <span>拍摄</span>
        </div>
      </div>
  ...
  methods: {
    ...
    sendMessage() {
      ...
      let data = {
        ...
        receive_id: this.member_id, //对方的memberid
        receive_type: 1,
        group_id: 0,
    ...
    receiveMessage(data) {
      ...
      if (
        data.group_id == 0 &&
        (this.loginId == data.receive_id || this.loginId == data.sent_id) &&
        (this.member_id == data.receive_id || this.member_id == data.sent_id)
      ) {
        this.message = "";
    ...
    getChatHistory() {
      ...
      Chat.getChatHistory(params).then(res => {
        if (this.page === 1) {
          if (res.data.length) {
